...
PARSE_SYSTEM_MESSAGE = {
    ...
    "content": PARSE_SM_TEMPLATE.format(
        sample_input=SAMPLE_PARSE_INPUT,
        sample_output=SAMPLE_PARSE_OUTPUT,
        none_found=NO_ENTITIES_MARKER,
    ),
}
...
def get_output_reservation(model):
    ...
    if model == "gpt-4o" or model == "gpt-4o-mini":
        ...
        return 16000  # Leave a small buffer
...
def get_timeout_limit(model):
    if model == "gpt-4o":
        return 300  # Higher timeout for larger context model
    elif model == "gpt-4o-mini":
        return 240  # Slightly lower timeout for mini version
    elif model == "gpt-3.5-turbo-16k":
        return 240
    elif model == "gpt-4":
        return 150
...
async def async_fetch_parse(
    ...
    if prompt_override:
        system_message = {"role": "system", "content": prompt_override}
    else:
    ...
    log_msg(
        f"Parse result length (in tokens): {get_token_length(parse_result, model=model)}"
    )
