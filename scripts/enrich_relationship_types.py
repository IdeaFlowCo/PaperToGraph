    ...
    with open(source_file, 'r') as f:
        ...
        for i, row in enumerate(reader, start=1):
            if row['type'] not in RELATIONSHIP_TYPES:
                log_warn(f'Source file line [{i}]: relationship type "{row["type"]}" not recognized, skipping')
                continue
    ...
    try:
        if args.source_file:
            ...
            if args.top_n:
                ...
                log_msg(f'--top_n set to {args.top_n}, retrieving types for {len(relationship_names)} relationships.')

            log_msg('Fetching name/type pairs from GPT...')
            name_type_pairs = await _get_rel_types_from_gpt(relationship_names, args.gpt_model)
